...
CREATE TABLE IF NOT EXISTS calc_runs(id INTEGER PRIMARY KEY, kind TEXT, trip_id INTEGER, input_json TEXT, result_json TEXT, created_at TEXT, plate TEXT);
CREATE TABLE IF NOT EXISTS vehicles(id INTEGER PRIMARY KEY, plate TEXT NOT NULL, enabled INTEGER NOT NULL DEFAULT 1);
"""
...
def init_db():
    ...
    if conn.execute("SELECT COUNT(*) c FROM tariff").fetchone()["c"] == 0:
        ...
        conn.execute("INSERT INTO vehicles(plate,enabled) VALUES ('沪A·00001',1)")
